...
//  float SCALEMU = 1.0/0.76;
//  float SCALEMU = 1.0/0.8;
 float SCALEMU = 1.0/1.0;
...
//  hs->addWeight(NY-minNY); //---- add S/(S+B) weight ---> used only for propaganda plot and data-background
 hs->addWeight1D(NY-minNY); //---- add S/(S+B) weight line-by-line ---> used only for propaganda plot and data-background 
...
//  hs->Draw(c1,1,true,true,c1bis, 315.4, 1); //--- last 1 = scaleEvenData --> scale 0.76
